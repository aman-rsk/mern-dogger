...
function HomeCard(props) {
	// We are using useSelector to get the state of the logged-in user
	let user = useSelector((state) => state.user);

	// We are using useNavigate from react-router-dom to navigate between pages
	const navigate = useNavigate();

	// We are using useState hook to handle the state of the Tweet details modal
	// show is initially false, it is used to toggle the visibility of the modal
	const [show, setShow] = useState(false);

	// handleClose is a function that sets the show state to false when called
	const handleClose = () => setShow(false);

	// handleShow is a function that sets the show state to true when called
	const handleShow = () => setShow(true);
	//This section of code is responsible for creating a comment section.
	...
	// Creating a 'config_obj' to configure the authorized user.
	const CONFIG_OBJ = {
		headers: {
	...
	// To display a "like" button if the post has not been liked, and display an "unlike" button if the post has already been liked.
	...
	function seeprofile() {
		navigate(`/otherprofile/${props.tweetData.author._id}`);
	}
	...
	const handleDeletReply = (replyId, commentId) => {
		try {
			axios
				.delete(`${API_BASE_URL}/reply/${replyId}`, CONFIG_OBJ)
				.then((response) => {
					Swal.fire({
						icon: 'success',
						title: 'Reply Deleted',
